...
export const {Playground, Fragments} = (() => {
	class Playground {
		...
		static createBrowserPlayground(container, target) {
			...
			// target.sandbox.add('allow-scripts', 'allow-presentation');
			// target.addEventListener('scroll', console.log);
			target.hidden = true;
			const placeholder = container.querySelector(':scope > output[here], :scope > * output[here]');
		...
		/** @param {Partial<Playground>} options */
...
/** @typedef {{hidden?: boolean, contentDocument: HTMLFrameElement['contentDocument'], contentWindow?: HTMLFrameElement['contentWindow'], srcdoc: string} | HTMLIFrameElement} PlaygroundTarget */

	...
	class MarkoutContent {
		...
		rewriteAnchors(...anchors) {
			...
			for (const anchor of anchors) {
				const [matched, dir, name, extension = '.md'] = RewritableURL.exec(anchor.getAttribute('href'));
				...
				if (dir) {
					...
					const href =
						name && extension.toLowerCase() === '.md'
							...
							: new URL(`${pathname}${search}`, sourceURL);

					debugging && console.log({matched, href, dir, name, extension});
					anchor.href = href;
				} else {
